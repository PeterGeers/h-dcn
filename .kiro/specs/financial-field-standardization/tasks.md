...
- [x] 1. Backend shared helper: validate_price_field
  - [x] 1.1 Create `backend/handler/shared/price_validation.py` with `validate_price_field` function
  ...
  - [x] 1.2 Write property test: Numeric inputs produce equivalent Decimal
  ...
  - [x] 1.3 Write property test: Non-numeric inputs are always rejected
  ...
  - [x] 1.4 Write property test: String-Decimal round-trip consistency
  ...
  - [x] 1.5 Write property test: Successful validation never returns NaN or Infinity
...
- [x] 2. Backend handler integration
  - [x] 2.1 Add validation to `admin_update_product/app.py`
  ...
  - [x] 2.2 Add validation to `admin_create_variant/app.py`
    - Validate `prijs` field if provided in request body
    - Return 400 with error message if validation fails
    - _Requirements: 3.2, 3.5_
  - [x] 2.3 Add validation to `admin_create_product/app.py`
    - Validate price field before storing
    - Return 400 with error message if validation fails
    - _Requirements: 3.3, 3.5_
  - [x] 2.4 Add validation to `create_order/app.py`
    - Validate price fields in line items
    - Return 400 with error message if any line item has non-numeric price
    - _Requirements: 3.4, 3.5_
  - [x] 2.5 Write unit tests for handler validation (all 4 handlers)
...
- [x] 4. Frontend inline pattern replacements
  - [x] 4.1 Replace inline pattern in `webshop/WebshopPage.tsx`
    - Replace `Number(item.price || 0).toFixed(2)` with `formatPrice(item.price)`
    - Add import for `formatPrice` from `../../utils/formatPrice`
    - _Requirements: 1.1, 1.5_
  - [x] 4.2 Replace inline pattern in `webshop/CheckoutModal.tsx`
  ...
  - [x] 4.3 Replace inline pattern in `webshop/OrderConfirmation.tsx`
    - Replace `Number(item.price || 0).toFixed(2)` with `formatPrice(item.price)`
    - Add import for `formatPrice`
    - _Requirements: 1.3, 1.5_
  - [x] 4.4 Replace inline pattern in `advanced-exports/AdvancedExportsPage.tsx`
    - Replace `productStats.gemiddeldePrijs.toFixed(2)` with `formatPrice(productStats.gemiddeldePrijs)`
    - Add import for `formatPrice`
    - _Requirements: 1.4, 1.5_

- [x] 5. Migration script
  - [x] 5.1 Create `scripts/migrate_price_fields_to_number.py`
...
- [x] 6. Steering rule update
  - [x] 6.1 Add section 6 to `.kiro/steering/schema-driven.md`
...
- [x] 8. Commit, push, and deploy
  - [x] 8.1 Commit all changes on current feature branch
  ...
  - [x] 8.2 Push and trigger deploy workflows (after user approval)
