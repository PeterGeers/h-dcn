...
try:
    ...
    lambda_handler = create_smart_fallback_handler("create_member")
    # Exit early - the fallback handler will handle all requests
    import sys
    sys.exit(0)

dynamodb = boto3.resource('dynamodb')
table_name = os.environ.get('DYNAMODB_TABLE', 'Members')
...
def lambda_handler(event, context):
    try:
        ...
        print(f"Member {member_id} created by {user_email} with roles {user_roles}")
