...
try:
    ...
    lambda_handler = create_smart_fallback_handler("upload_club_logo")
...
def lambda_handler(event, context):
    try:
        ...
        log_successful_access(user_email, user_roles, 'upload_club_logo')
        ...
        if not club_id:
            return create_error_response(400, 'Missing required field: club_id')
        if not content_type:
        ...
        # Authorization: allow if club_id matches OR user has admin role
        has_admin_role = bool(ADMIN_ROLES.intersection(set(user_roles)))
        if club_id != user_club_id and not has_admin_role:
            return create_error_response(
                403, 'Not authorized to upload logo for this club'
            )
        ...
        print(f"Error in upload_club_logo handler: {str(e)}")
