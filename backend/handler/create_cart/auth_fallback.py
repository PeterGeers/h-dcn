"""
Fallback authentication module for create_cart handler
...
def validate_permissions(user_roles, required_permissions, user_email=None, resource_context=None):
    ...
    try:
        ...
        if any(role in special_roles for user_roles):
            # For cart operations, hdcnLeden has webshop access
            if 'hdcnLeden' in user_roles and 'webshop_access' in required_permissions:
                print(f"✅ Webshop access granted for hdcnLeden user {user_email}")
                return True, None
            
            # Other special roles have limited access - deny by default for admin functions
