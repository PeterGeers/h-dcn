...
Consolidated from handler-local copies into the shared layer for single-source-of-truth.
"""
...
ORGANIZATIONAL_ROLE_COMBINATIONS = {
    ...
    'Regional_Chairman_Region1': [
        ...
        'Products_Read', 'Regio_Noord-Holland',
    ...
    'Regional_Treasurer_Region7': [
        ...
        'Products_Read', 'Regio_Groningen/Drenthe'
    ],
...
def convert_legacy_roles_to_new_structure(roles):
    """
    Convert legacy _All roles to the new permission + region structure
    
    Args:
        roles (list): List of role names (may include legacy _All roles)
        
    Returns:
        dict: {
            'converted_roles': list,  # New role list with legacy roles replaced
            'removed_roles': list,    # Legacy roles that were removed
            'added_roles': list       # New roles that were added as replacements
        }
    """
    # Mapping of legacy _All roles to their new equivalents
    legacy_mapping = {
    ...
    for role in roles:
        if role in legacy_mapping:
            ...
            for new_role in legacy_mapping[role]:
                if new_role not in converted_roles:
                    converted_roles.append(new_role)
    ...
    return {
        'converted_roles': converted_roles,
