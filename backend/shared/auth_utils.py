...
def validate_permissions_with_regions(user_roles, required_permissions, user_email=None, resource_context=None):
    ...
    try:
        ...
        # Exception: System roles and basic member roles don't need region roles
        system_roles = ['System_CRUD', 'System_User_Management', 'System_Logs_Read']
        basic_member_roles = ['hdcnLeden', 'verzoek_lid']  # Basic member roles for webshop access
        has_system_role = any(role in user_roles for role in system_roles)
        has_basic_member_role = any(role in user_roles for role in basic_member_roles)
        
        if not has_system_role and not has_basic_member_role:
        ...
        # If we get here, user has both permission and region roles (or is system/basic member user)
        regional_info = determine_regional_access(user_roles, resource_context)
        return True, None, regional_info
...
def validate_permissions(user_roles, required_permissions, user_email=None, resource_context=None):
    ...
    try:
        ...
        role_permissions = {
            ...
            'Members_Export': ['members_export', 'members_read'],  # Export users need read access to download files
...
def has_permission_and_region_access(user_roles, required_permission_type, required_regions=None):
    ...
    Returns:
        dict: {
            ...
            'region_roles': list,
            'access_type': str,  # 'admin', 'new_structure', 'denied'
            'message': str
        }
    """
