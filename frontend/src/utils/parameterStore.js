...
class ParameterStore {
  ...
  async convertApiToFormStructure() {
    const formStructure = {};
  ...
  getFlatArray(category) {
    ...
    return Object.entries(data).flatMap(([key, item]) => {
      ...
      const children = item.children ? Object.values(item.children).map(child => ({
        id: child.id, 
        value: child.value, 
        parent: item.id
      })) : [];
...
const useParameterLoader = (category, dataGetter) => {
...
export const useHierarchicalParameters = (category) => {
