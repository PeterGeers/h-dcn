...
const ProductFilter = ({ products, selectedFilter, onFilterChange }: ProductFilterProps) => {
  ...
  const GroupItem = ({ groupData }: GroupItemProps) => {
    ...
    return (
      <Box>
        <Button
          ...
          bg={isGroupSelected ? 'brand.orange' : 'transparent'}
          color={isGroupSelected ? 'white' : 'inherit'}
          _hover={{ bg: isGroupSelected ? 'brand.orange' : 'gray.600' }}
        >
          <Box flex={1} textAlign="left" onClick={(e: React.MouseEvent) => {
            e.stopPropagation();
            onFilterChange({ type: 'group', value: groupData.group });
          }}>
            {groupData.group}
              ...
              {groupData.subgroups.map(subgroup => {
                ...
                return (
                  <Button
                    ...
                    bg={isSubgroupSelected ? 'brand.orange' : 'transparent'}
                    color={isSubgroupSelected ? 'white' : 'gray.300'}
                    _hover={{ bg: isSubgroupSelected ? 'brand.orange' : 'gray.600' }}
                  >
                    {subgroup}
                  </Button>
                );
  ...
  return (
    <Box bg="brand.gray" borderRadius="md" mb={4} color="white" w={{ base: 'full', lg: '300px' }}>
      {/* Dropdown Header */}
      <Button
        onClick={onToggle}
        variant="ghost"
      ...
      <Collapse in={isOpen}>
        <Box p={4} pt={2}>
          <Button
            ...
            bg={!selectedFilter ? 'brand.orange' : 'transparent'}
            color={!selectedFilter ? 'white' : 'gray.300'}
            _hover={{ bg: !selectedFilter ? 'brand.orange' : 'gray.600' }}
            w="full"
            justifyContent="flex-start"
          >
            Alle producten
