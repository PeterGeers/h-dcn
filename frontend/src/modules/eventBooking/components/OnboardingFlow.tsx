...
import { ClubRegistryEntry, ClubRegistry } from '../types/presmeet.types';
...
const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ onComplete, eventId, userClubId }) => {
  ...
  useEffect(() => {
    const fetchRegistry = async () => {
      ...
      const response = await ApiService.get<ClubRegistry>('/presmeet/clubs');
  ...
  const handleSelectClub = async (club: ClubRegistryEntry) => {
    ...
    const response = await ApiService.post<{ message: string; club_id: string }>('/presmeet/clubs/assign', { club_id: club.club_id });
      ...
      <Alert status="error" borderRadius="md">
        ...
        <Button
          ...
          onClick={() => {
            setFetchError(null);
            setLoading(true);
            ApiService.get<ClubRegistry>('/presmeet/clubs').then((response) => {
              if (response.success && response.data) {
                setRegistry(response.data);
              } else {
                setFetchError(response.error || 'Failed to load club registry');
