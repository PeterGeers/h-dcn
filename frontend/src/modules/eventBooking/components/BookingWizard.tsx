...
const BookingWizard: React.FC<BookingWizardProps> = ({ eventId, delegateName, onSubmitted }) => {
  ...
  const handleSave = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const items = formStateToOrderItems(formState, products);
      const updatedOrder = await eventBookingApi.saveOrder(order.order_id, {
        items,
        version,
      });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const items = formStateToOrderItems(formState, products);
      const savedOrder = await eventBookingApi.saveOrder(order.order_id, {
        items,
        version,
      });
      ...
      // Then submit
      const submittedOrder = await eventBookingApi.submitOrder(order.order_id);
