...
import { Event, Constraint, ReportResponse, Product } from '../types/eventBooking.types';
...
const EventDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      try {
        const eventList = await eventBookingApi.getEvent('presmeet');
        setEvents(eventList);
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [overviewResult, financialResult] = await Promise.all([
        eventBookingApi.getReport({ type: 'overview', event_id: selectedEventId }),
        eventBookingApi.getReport({ type: 'financial', event_id: selectedEventId }),
      ]);
      ...
      if (productIds.length > 0) {
        const productList = await eventBookingApi.getProducts(selectedEventId, productIds);
