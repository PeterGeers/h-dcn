...
function AnalyticsDashboard({ events }) {
  const analytics = useMemo(() => {
    ...
    const totalParticipants = events.reduce((sum, event) => sum + (parseInt(event.participants || event.aantal_deelnemers) || 0), 0);
    const averageAttendance = totalEvents > 0 ? Math.round(totalParticipants / totalEvents) : 0;
    
    const totalRevenue = events.reduce((sum, event) => sum + (parseFloat(event.revenue || event.inkomsten) || 0), 0);
    const totalCosts = events.reduce((sum, event) => sum + (parseFloat(event.cost || event.kosten) || 0), 0);
    ...
    const profitableEvents = events.filter(event => 
      (parseFloat(event.revenue || event.inkomsten) || 0) > (parseFloat(event.cost || event.kosten) || 0)
    ).length;
    ...
    events.forEach(event => {
      ...
      if (eventDate) {
        ...
        monthlyData[month].participants += parseInt(event.participants || event.aantal_deelnemers) || 0;
        monthlyData[month].revenue += parseFloat(event.revenue || event.inkomsten) || 0;
        monthlyData[month].costs += parseFloat(event.cost || event.kosten) || 0;
      }
    });
    ...
    events.forEach(event => {
      ...
      if (location) {
        ...
        locationData[location].participants += parseInt(event.participants || event.aantal_deelnemers) || 0;
        locationData[location].revenue += parseFloat(event.revenue || event.inkomsten) || 0;
        locationData[location].costs += parseFloat(event.cost || event.kosten) || 0;
      }
    });
