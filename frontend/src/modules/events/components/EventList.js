...
function EventList({ events, onEventUpdate, user }) {
  ...
  const handleDelete = async (event) => {
    ...
    try {
      const response = await fetch(`https://i3if973sp5.execute-api.eu-west-1.amazonaws.com/prod/events/${event.event_id}`, {
        method: 'DELETE'
      });
  ...
  const getEventField = (event, field) => {
    ...
    const fieldMap = {
      ...
      aantal_deelnemers: event.participants || event.aantal_deelnemers || event.max_participants || 0,
