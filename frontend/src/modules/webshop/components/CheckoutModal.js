...
const CheckoutForm = ({ cartItems, totalAmount, deliveryOptions, selectedDelivery, onDeliveryChange, onSuccess, onError, userEmail }) => {
  const stripe = useStripe();
  const elements = useElements();
  const [isProcessing, setIsProcessing] = useState(false);
  const [sameAsUserAddress, setSameAsUserAddress] = useState(true);
...
const CheckoutModal = ({ isOpen, onClose, cartItems, onPaymentSuccess, userEmail }) => {
  ...
  const handleSuccess = (paymentData) => {
    ...
    setError(null);
    setTimeout(() => {
      // Validate and sanitize payment data to prevent code injection
