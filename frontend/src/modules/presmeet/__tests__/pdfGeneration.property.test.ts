...
let capturedTextCalls: string[] = [];
let capturedAutoTableCalls: Array<{ head: string[][]; body: string[][] }> = [];
...
jest.mock('jspdf-autotable', () => ({
  ...
  default: (doc: any, options: any) => {
    ...
    if (options?.head && options?.body) {
      capturedAutoTableCalls.push({ head: options.head, body: options.body });
    }
  },
}));
...
function getAllPdfText(): string {
  const directText = capturedTextCalls.join(' ');
  const tableText = capturedAutoTableCalls
