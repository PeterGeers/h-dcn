...
const validationSchema = Yup.object().shape({
  ...
  minderjarigNaam: Yup.string().when('geboortedatum', {
    is: (val) => val && Math.floor((new Date() - new Date(val)) / (365.25 * 24 * 60 * 60 * 1000)) < 18,
...
function MembershipForm({ user }) {
  ...
  const [existingMember, setExistingMember] = React.useState(null);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      const url = existingMember 
        ? `https://i3if973sp5.execute-api.eu-west-1.amazonaws.com/prod/members/${existingMember.member_id}`
        : 'https://i3if973sp5.execute-api.eu-west-1.amazonaws.com/prod/members';
      ...
      toast({
        title: existingMember ? 'Gegevens bijgewerkt' : 'Aanmelding verzonden',
        description: existingMember ? 'Je gegevens zijn succesvol bijgewerkt.' : 'Je aanmelding is succesvol opgeslagen.',
        status: 'success',
        duration: 5000
      });
        ...
        >
          {({ errors, touched, isSubmitting, values }) => {
            ...
            const isUnder18 = values.geboortedatum && 
              Math.floor((new Date() - new Date(values.geboortedatum)) / (365.25 * 24 * 60 * 60 * 1000)) < 18;
