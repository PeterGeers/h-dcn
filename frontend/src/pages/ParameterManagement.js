...
const convertToFlatForSave = (data) => {
  const flat = { ...data };
...
function ParameterManagement({ user }) {
  ...
  const convertAllApiToFormStructure = async (apiData) => {
    const formStructure = {};
      ...
      apiData.forEach(param => {
        try {
          ...
          if (name === 'productgroepen') {
            ...
            const flatArray = [];
            Object.entries(nestedData).forEach(([key, item]) => {
              flatArray.push({
                id: item.id,
                value: item.value,
                parent: null
  ...
  const openModal = (item = null, parent = null) => {
    setEditingItem(item);
  ...
  const getChildren = (parentId) => currentParameters.filter(p => p.parent === parentId);
